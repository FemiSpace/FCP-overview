...
export const STREAMS: Stream[] = [
  {
    ...
    inputs:[
      ...
      {id:'cycles',     label:'Charge/discharge cycles per day',   unit:'cycles', bear:1.0,  base:1.5,  bull:2.0,
       note:'RESEARCHED & CORRECTED from 2.0 base. Modo Energy 2024: ERCOT Top-Bottom 1hr spreads averaged $98/MWh — a 61% drop from 2023. Battery saturation compressing short-duration spreads. 24-hr UPHES has structural advantage but realistic cycles depend on ERCOT dispatch modelling.',
       confirmed:false,
      ...
      {id:'avail',      label:'Plant availability factor',         unit:'%',      bear:0.79, base:0.83, bull:0.90,
       note:'RESEARCHED & CORRECTED from 95%. DOE Hydropower Market Report 2023: actual US hydro availability was stable at 79% (small units), 83% (medium), 78% (large) from 2019-2021. New plant with modern controls may achieve higher — flagged as upside. 95% original was unsourced.',
       confirmed:false,
      ...
      {id:'sell_price', label:'Peak sell price (ERCOT 2032 est.)', unit:'$/MWh',  bear:55,   base:75,   bull:120,
       note:'RESEARCHED & CORRECTED from $88. ERCOT 2024 annual average: $26/MWh (Modo Energy). EIA 2025 forecast: $27-34/MWh average. Summer on-peak July-Aug 2025 forwards: $110-165/MWh (S&P Global). $75/MWh base for 2032 assumes moderate recovery from data center load growth. $88 original was not based on any source.',
       confirmed:false,
      ...
      {id:'buy_price',  label:'Off-peak buy price (ERCOT 2032 est.)', unit:'$/MWh', bear:15, base:28,   bull:35,
       note:'RESEARCHED & CORRECTED from $20. EIA projects ERCOT avg $27-34/MWh for 2025. Off-peak solar suppression hours can go near zero but average pumping window is $15-28/MWh. $20 original was too low — not based on any source.',
       confirmed:false,
    ...
    outputs:[
      {label:'Annual arbitrage volume',  formula:'Grid MW × Alloc% × Cycles × Duration × RT_Eff × Avail × 365 ÷ 1,000', unit:'GWh/yr',
       ...
       note:'CORRECTED — cycles/day reduced from 2.0 to 1.5 base'},
      {label:'Net spread per MWh',       formula:'Sell price − (Buy price ÷ RT_Efficiency)',                                unit:'$/MWh',
       bearVal:'$36',   baseVal:'$40',   bullVal:'$96',
       note:'CORRECTED: = $75 − ($28 ÷ 0.80) at base. Prior $63/MWh was based on unresearched inputs.'},
      {label:'GROSS ARBITRAGE REVENUE',  formula:'Volume GWh × Net Spread × 1,000',                                         unit:'$000s',
       ...
       note:'CORRECTED — significantly lower than prior $352M. Needs full ERCOT dispatch model to validate.'},
    ],
    assumptions:[
      {param:'⚠ CRITICAL FLAG',
...

  {
    ...
    inputs:[
      ...
      {id:'ppa_rate',label:'PPA rate — certified 24/7 clean',  unit:'$/MWh', bear:70,  base:85,   bull:110,
       note:'RESEARCHED & CORRECTED from $95. ERCOT wholesale avg ~$30/MWh + green reliability premium $40-48/MWh (McKinsey/LDES 2022) = $70-78/MWh floor for 24/7 CFE from hybrid systems. Hybrid 24/7 PPA costs >$200/MWh (McKinsey 2022). FCP\'s firm hydro at $85/MWh is dramatically cheaper and genuinely dispatchable — defensible base.',
       confirmed:false,
...

  {
    ...
    inputs:[
      ...
      {id:'as_price',    label:'AS clearing price (ERCOT blended)',  unit:'$/MW/hr', bear:2.5,  base:4.5,  bull:6.5,
       note:'RESEARCHED & CORRECTED from $5. ECRS averaged $4.74/MW/hr in Jul-Aug 2024 — down 29% YoY (Modo Energy Aug 2024). Blended ECRS + Non-Spin + RRS. Declining trend as battery fleet expands in ERCOT.',
       confirmed:false,
...

  {
    ...
    inputs:[
      ...
      {id:'pk_price', label:'Peak dispatch price (ERCOT)',      unit:'$/MWh', bear:80,   base:120,  bull:180,
       note:'RESEARCHED & CORRECTED from $130. S&P Global April 2025: ERCOT Jul-Aug on-peak forward curves at $110-165/MWh for 2025. $120/MWh base for 2032 is reasonable given data center load growth expected to tighten summer peaks.',
       confirmed:false,
...
export const CAPEX_ITEMS: CapexItem[] = [
  ...
  {category:'Solar farm', item:'600 MWp solar farm (co-located)',
   ...
   benchmark:'CORRECTED & SOURCED. Wood Mackenzie/SEIA Q4 2025: utility-scale solar costs climbed to $1.18/Wdc (fixed-tilt) and $1.35/Wdc (single-axis tracker), up 11-14% YoY due to Trump tariffs on steel and aluminium. 600 MWp × $1.18-1.35/Wdc = $708-810M. Original $420M base was based on outdated 2023 pre-tariff pricing and is materially wrong.',
   benchmarkUrl:'https://seia.org/research-resources/solar-market-insight-report-2025-year-in-review/',
   watch:'🔴 MAJOR CORRECTION: Solar cost at 2025 prices is $708-810M — nearly double the original $420M assumption. Also: does FCP OWN the solar farm (adds $700M+ to capex) or buy under a PPA (pays $/MWh opex)? Solar ownership alone would nearly double total project capex. Must decide before M3.'},
...
export const PROJECT_OPEX: OpexItem[] = [
  {category:'Operations & maintenance', item:'Annual O&M — PHES components',
   ...
   benchmark:'CORRECTED & SOURCED. IRENA benchmark: O&M costs for hydropower 1-4% of annual investment costs. Large hydro averages 2-2.5% (IRENA Hydropower Technology Brief, ETSAP). Base raised from 1% to 1.5% — original 1% was optimistic low end with no source.',
  ...
  {category:'Energy input', item:'Off-peak electricity purchase for pumping',
   ...
   benchmark:'CORRECTED. Buy price raised from $20 to $28/MWh base (EIA 2025 ERCOT forecast: $27-34/MWh average). At corrected 4,194 GWh/yr pumping volume × $28/MWh = ~$117M/yr. If FCP owns solar, pumping cost approaches zero marginal cost.',
